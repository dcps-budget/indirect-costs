...
d_485_simple <-
  ...
  mutate(across(c(starts_with("amt_")), \(x) (as.numeric(x)))) %>%
...
d_485_long <-
  d_485_simple %>%
  pivot_longer(cols = amt_x7011001:amt_x7016001, names_to = "account", values_to = "amt") %>%
  mutate(across(account, \(x) str_remove(x, "amt_x")))
```

```{r}
n_rows_485 <- nrow(data_485)
...
adjustments_by_account <-
  ...
  filter(account != "7012999") %>%
  mutate(rec_multiplier = if_else(is.na(amt_485) | amt_485 == 0.00, 1.00, ending_balance / amt_485)) %>%
  mutate(rec_addon = if_else(is.na(amt_485) | amt_485 == 0.00, ending_balance / n_rows_485, 0.00)) %>%
  select(account, rec_addon, rec_multiplier)

d_485_corrected_long <-
  d_485_long %>%
...
d_485_corrected <-
  ...
  group_by(
    costcenter, costcenter_name, program, program_name, position, emplid, name,
    pp_485, group_485, date_485,
    account_col
  ) %>%
  summarize(amt_corrected = sum(amt_corrected)) %>%
  ungroup() %>%
  pivot_wider(id_cols = c(costcenter:date_485), names_from = account_col, values_from = amt_corrected, names_sort = TRUE)
