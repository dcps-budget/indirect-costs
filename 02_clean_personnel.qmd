...
position_titles <-
  ...
  slice_max(order_by = pay_end_dt, n = 1) %>%
  ungroup() %>%
...
For these missing accounts, additional rows will be imputed into the 485.
...
attributes_imputable <-
  ...
  distinct(
    fund_id, program_id, costcenter_id, project_id, award_id,
    account_id
  ) %>%
...
rows_need_impute_general <-
  data_485 %>%
  select(!c(account_id, amount)) %>%
  distinct() %>%
  mutate(account_id = "7014010")
